...
import (
	...
	"github.com/decimalteam/ethermint/tests"
...
func (suite AnteTestSuite) TestAnteHandler() {
	...
	setup := func() {
		...
		suite.app.EvmKeeper.SetBalance(suite.ctx, addr, uint256.NewInt(10000000000))
...
func (suite AnteTestSuite) TestAnteHandlerWithDynamicTxFee() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.ctx = suite.ctx.WithIsCheckTx(tc.checkTx).WithIsReCheckTx(tc.reCheckTx)
			suite.app.EvmKeeper.SetBalance(suite.ctx, addr, uint256.NewInt((ethparams.InitialBaseFee+10)*100000))
			_, err := suite.anteHandler(suite.ctx, tc.txFn(), false)
			if tc.expPass {
				suite.Require().NoError(err)
			} else {
