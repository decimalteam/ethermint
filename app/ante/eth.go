...
import (
	...
	"github.com/decimalteam/ethermint/x/evm/keeper"
...
func (egcd EthGasConsumeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	if ctx.IsReCheckTx() {
		...
		// https://github.com/decimalteam/ethermint/issues/1554
...
func (ctd CanTransferDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, msg := range tx.GetMsgs() {
		...
		valueConv, _ := uint256.FromBig(coreMsg.Value)

		// check that caller has enough balance to cover asset transfer for **topmost** call
		// NOTE: here the gas consumed is from the context with the infinite gas meter
		if coreMsg.Value.Sign() > 0 && !evm.Context().CanTransfer(stateDB, coreMsg.From, valueConv) {
			return ctx, errorsmod.Wrapf(
