...
case $PROTO_DIR_TO_GENERATE in
   "ethermint")
   echo "Generate $PROTO_LANG code to ethermint dir"
   generateProtoFiles $BUF_CONFIG "ethermint"
      ;;
   "all-proto")
   echo "Generate $PROTO_LANG code to ethermint directories"
   generateProtoFiles $BUF_CONFIG "ethermint"
      ;;
   *) echo "ERROR: Got unknown proto dir path $PROTO_DIR_TO_GENERATE (ethermint or all-proto required)"
     exit 1
      ;;
esac
